...
//! carbon-negative, and ecologically-restorative industrial machinery.[file:3][file:7]
//!
//! # Architecture
//!
//! This library implements a multi-plane risk assessment system where each
//! operational domain (energy, hydraulics, biology, carbon, materials, etc.)
...
//! governs all actuation decisions.[file:3][file:7]
//!
//! # Safety Guarantees
//!
//! - No action without a risk estimate (enforced at type level)[file:7]
//! - Lyapunov stability invariant: V_{t+1} ≤ V_t + ε[file:3]
//! - Hard corridor gates prevent instantiation of unsafe configurations[file:7]
//!
//! # Governance Metrics
//!
//! K/E/R triad provides rolling-window assessment:[file:3][file:7]
...
/// Identifies each normalized risk plane in the multi-dimensional safety space.[file:3][file:7]
...
/// Corridor bands shared across all planes (safegoldhard pattern).[file:3][file:7]
...
/// Dimensionless risk coordinate r∈[0,1] with attached corridor metadata.[file:7]
...
/// Piecewise-linear corridor normalization into r∈[0,1].[file:7]
pub fn normalize(raw: f64, min: f64, max: f64, bands: CorridorBands) -> RiskCoord {
    ...
    // Nonlinear scaling: safe/gold/hard shaping.[file:7]
...
/// Residual state: Lyapunov scalar + fixed-plane coordinates (E,H,B,C,M,Th,Mec,Sens).[file:3]
#[derive(Clone, Copy, Debug)]
...
/// Rolling-window KER metrics over a controller trajectory.[file:3][file:7]
#[derive(Clone, Copy, Debug)]
pub struct KerTriad {
    pub k_knowledge: f64,
    pub e_ecoimpact: f64,
...
impl KerWindow {
    ...
    pub fn triad(&self) -> KerTriad {
        ...
        let e = (1.0 - r).clamp(0.0, 1.0);[file:3][file:7]
...
/// Recompute V_t = Σ w_j r_j^2 over all configured planes.[file:3][file:7]
...
/// Corridor / Lyapunov decision used by controllers and routers.[file:3][file:21]
...
/// Enforce non‑increasing V_t outside a designated safe interior, with hard‑band stop at r=1.[file:3][file:7]
pub fn safe_step(prev: &ResidualState, next: &ResidualState, eps_vt: f64) -> CorridorDecision {
    // Hard stop if any coordinate hits 1.0 exactly.[file:21]
...
// ============================================================================
// RISK VECTOR (plane-indexed) AND SYSTEM STATE
// ============================================================================

/// Complete risk assessment vector containing all normalized risk coordinates.[file:3][file:7]
#[derive(Debug, Clone)]
...
/// Complete system state snapshot for controller decision-making.[file:3][file:7]
#[derive(Debug, Clone)]
...
/// Operating modes for Cyboquatic machinery.[file:3]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// Trait that all Cyboquatic controllers must implement.[file:7]
///
...
/// Rolling-window governance metrics (Knowledge/Eco-impact/Risk).[file:3][file:7]
#[derive(Debug, Clone)]
...
/// Central enforcer of Lyapunov stability contracts (non-actuating).[file:3][file:21]
...
/// Errors that can occur during ecosafety enforcement.[file:3][file:7]
...
pub fn aggregate_risk(risk_vector: &RiskVector, weights: &[f64; MAX_RISK_PLANES]) -> f64 {
    ...
    for i in 0..MAX_RISK_PLANES {
        ...
        aggregate += w * risk_vector.get_coordinate(unsafe { core::mem::transmute::<u8, RiskPlane>(i as u8) });
        w_sum += w;
    }
    if w_sum > 0.0 {
        aggregate / w_sum
